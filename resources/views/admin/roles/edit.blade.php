        ...
        <div class=" wizard-modern modern-wizard-example">
            <div class="bs-stepper-header">
                <form class="form" method="post" action="{{ ROUTE('roles.update',$role->id) }}"
                    style="    width: 100%; padding: 3rem 2rem; border-radius: 10px;">
                    @csrf
                    @method('PUT')
                    <div id="account-details-modern" class="content" role="tabpanel"
                        ...
                        <div class="content-header">
                            ...
                            <small class="text-muted">Enter Your Role Info.</small>
                        </div>
                        <br>
                        <div class="row">
                                    ...
                                    <div class="row">
                                        @foreach ($item as $key => $permission)
                                            <div class="mb-1 col-4">
                                                <label for="name"
                                                    class="form-check-label mx-auto">{{ $permission->name }}
                                                </label>
                                               
                                                <input type="checkbox" class="form-check-input" name="permissions[]"
                                                    value="{{ $permission->id }}" @if(count($role->permissions)  && in_array($permission->id ,$role->permissions->pluck('id')->toArray())) checked 
